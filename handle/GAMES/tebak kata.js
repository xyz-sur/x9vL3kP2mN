import { removeUser, getUser, isUserPlaying } from '../../database/temporary_db/tebak kata.js';
...
async function process(sock, messageInfo) {
  ...
  if (isUserPlaying(remoteJid)) {
    ...
    if (fullText.toLowerCase() === data.answer) {
      ...
      // Mencari pengguna
      const user = await findUser(senderLid, 'tebak kata game');
      ...
      if (mess.game_handler.tebak_kata) {
        const messageNotif = mess.game_handler.tebak_kata.replace('@hadiah', hadiah);
        await sock.sendMessage(
          remoteJid,
          {
            text: messageNotif,
